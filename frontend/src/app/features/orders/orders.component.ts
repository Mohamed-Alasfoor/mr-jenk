...
export class OrdersComponent implements OnInit {
  ...
  confirm(o:Order){this.commerce.updateStatus(o.id,'CONFIRMED').subscribe({next:x=>{o.status=x.status;this.toast.show('Order confirmed','success');},error:()=>this.toast.show('Could not confirm order','error')});}
  canCancel(o:Order){return o.status==='PENDING';}
}
